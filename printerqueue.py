...
class ArrayQueue():
    ...
    def __repr__(self):
        return f"<ArrayQueue head={self._data[self._front]} length={self._length}>"
...
if __name__ == '__main__':
    ...
    def add_jobs(documents):
        """function to take in a list of documents as strings, and return a new queue instance with all the documents in the list added"""
        queued = Queue(list(str(documents)))

        return queued
    
    docs_list = ['employees.txt', 'file.md', 'index.txt']

    print(add_jobs(docs_list))
